...
if [[ "$(cat .token)" && "$REMOTE_URL" == https://* ]]; then
  git push -u "https://${GIT_USER}:$(cat .npmtoken)@${REMOTE_URL#https://}" HEAD --follow-tags
else
  git push --follow-tags
fi

if [[ "$MESSAGE" =~ -no-ci$ ]]; then
  echo "Publishing package to GitHub Packages (private)..."
  NPM_TOKEN=$(cat .token) npm publish --registry=https://npm.pkg.github.com --access restricted
fi

#npm run docker:login && npm run docker:build && npm run docker:publish
