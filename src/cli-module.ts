...
const logger = Logging.for("lavajet");
...
function ensureMigrationLockFile(): void {
  const lockFile = path.resolve(
    process.env.MIGRATIONS__LOCK_FILE || "./locks/version.lock"
  );
  let current: string | undefined;
  try {
    current = fs.readFileSync(lockFile, "utf-8").trim() || undefined;
  } catch {
    // missing -> first boot
  }
  if (current && SEMVER_RE.test(current)) return;
  const pkg = JSON.parse(
    fs.readFileSync(path.join(process.cwd(), "package.json"), "utf-8")
  );
  fs.mkdirSync(path.dirname(lockFile), { recursive: true });
  fs.writeFileSync(lockFile, `${pkg.version}\n`, "utf-8");
  const log = logger.for("boot");
  log.info(
    `Migration lock ${lockFile} was ${current ? `invalid ("${current}")` : "missing"} - seeded with package version ${pkg.version}`
  );
}
...
function runEntrypoint(script: string, execArgv: string[] = []): Promise<void> {
...
export const bootCommand = new Command()
  .name("boot")
  .description("boots the app entrypoint by delegating to node lib/main")
  .option(
    "--inspect [String]",
    "starts the app process with a node inspector bound to the given address:port (e.g. 0.0.0.0:9229). A bare flag defaults to 0.0.0.0:9229",
    false
  )
  .action(async (options: any) => {
    const input = resolveBootInputPath();
    const log = logger.for("boot");
    ensureMigrationLockFile();
    const execArgv: string[] = [];
    if (options.inspect) {
      const address =
        typeof options.inspect === "string" ? options.inspect : "0.0.0.0:9229";
      execArgv.push(`--inspect=${address}`);
      log.info(`App debugger enabled on ${address}`);
    }
    log.info(`Booting entrypoint from ${input}`);
    await runEntrypoint(input, execArgv);
  });

const migrateCommand = new Command()
  .name("migrate")
  .description(
    "boots the app context headlessly, executes lavajet migrations, and exits"
  )
...
const lavajetCmd = new Command()
  .name("nest")
  .description("exposes various commands to help manage the nest integration");

lavajetCmd.addCommand(bootCommand);
lavajetCmd.addCommand(migrateCommand);
lavajetCmd.addCommand(exportApiCommand);

export default function lavajet() {
  return lavajetCmd;
}
